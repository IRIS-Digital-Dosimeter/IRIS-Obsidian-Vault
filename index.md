...
5. Since SD cards writes have noticeable latency, we must buffer the incoming data for efficiency's sake.
...
ADC_BUF["ADC Result Buffers (circular, half-buffer)"]

WBUF["Interleaved Write Buffer"]
...
DMAC -->|"DMA sequence complete → trigger conversion"| ADCs
ADCs-->|"conversion result"| DMAC
DMAC -->|"write half-buffers (circular)"| ADC_BUF

CPU -->|"interleave data"| WBUF

CPU -->|"commit write buffer"| SD
DMAC -->|"half-buffer ready"| CPU

```

> [!info]- More Detailed Diagram
>```mermaid
>sequenceDiagram
>	participant ADC1
>	participant ADC2
